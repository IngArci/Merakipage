...
export function ProjectCard({ title, location, sizes, image, slug, status }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <Link to={`/proyectos/${slug}`} className="flex flex-col h-full">
        ...
        <div className="p-6 flex flex-col flex-grow bg-[var(--grad-glass)]">
          ...
          <Button
            className="w-full bg-gradient-to-r from-[var(--gold-1)] via-[var(--gold-5)] to-[var(--gold-1)] hover:brightness-110 text-black shadow-lg mt-auto font-bold tracking-widest text-xs h-12 rounded-xl transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(244,186,62,0.3)] pointer-events-none"
          >
            EXPLORAR PROYECTO
          </Button>
        </div>
